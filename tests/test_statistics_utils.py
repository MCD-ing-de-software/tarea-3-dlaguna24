...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        calculated_avg = utils.moving_average(input_numbers, window=2)

        expected_avg = np.array([1.5, 2.5, 3.5])

        npt.assert_allclose(calculated_avg, expected_avg, rtol=1e-7, atol=1e-7)
        self.assertEqual(calculated_avg.shape, expected_avg.shape)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        with self.assertRaises(ValueError):
            utils.moving_average(input_numbers, window=0)

        with self.assertRaises(ValueError):
            utils.moving_average(input_numbers, window=5)
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        multidimensional_input = [[1, 2], [3, 4]]

        with self.assertRaises(ValueError):
            utils.moving_average(multidimensional_input, window=2)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        zscore_result = utils.zscore(input_numbers)
        self.assertAlmostEqual(zscore_result.mean(), 0.0, places=7) 
        self.assertAlmostEqual(zscore_result.std(), 1.0, places=7)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        constant_values = [5, 5, 5]

        with self.assertRaises(ValueError):
            utils.zscore(constant_values)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        input_numbers = [2, 4, 6]
        scaled_result = utils.min_max_scale(input_numbers)
        expected_scaled_values = np.array([0.0, 0.5, 1.0])
        self.assertAlmostEqual(scaled_result.min(), 0.0, places=7)
        self.assertAlmostEqual(scaled_result.max(), 1.0, places=7)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        constant_values = [3, 3, 3]

        with self.assertRaises(ValueError):
            utils.min_max_scale(constant_values)
